...
@pytest.fixture(autouse=True)
def mock_console_width() -> None:
    """Ensure a consistent wide console for test outputs to prevent unexpected wrapping."""
    from ddgo_search import formatting, cli

    formatting.console.width = 150
    cli.console.width = 150
...
def test_format_simple_table() -> None:
    ...
    table = format_simple_table(headers, rows, width=80)
    expected = (
        "Name         Age      City\n"
        "---------------------------------\n"
        "Alice        24       New York\n"
...
def test_format_simple_table_no_truncation_and_wrap() -> None:
    ...
    headers = ["Name", "LongText"]
    rows = [
        [
            "Alice",
            "This is an extremely long string that will definitely wrap across multiple lines because the console width is set to a small value, but under no circumstances should it be truncated with an ellipsis.",
        ]
    ]
    # Set a small width (e.g. 50 characters) to force wrapping
    table = format_simple_table(headers, rows, width=50)

    # 1. Verify that the table output is wrapped (contains multiple lines)
    assert len(table.splitlines()) > 3

    # 2. Verify that there is NO ellipsis (...) generated by the formatter
    assert "..." not in table

    # 3. Verify all original words are present in the output
    assert "extremely" in table
    assert "circumstances" in table
    assert "truncated" in table


def test_print_table_news_no_truncation(capsys: pytest.CaptureFixture[str]) -> None:
    """Test that print_table_news prints complete news fields without truncation."""
    ...
    # Verify that the formatter itself did not insert any ellipses
